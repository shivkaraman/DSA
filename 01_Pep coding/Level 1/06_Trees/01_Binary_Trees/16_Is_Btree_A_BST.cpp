...
BSTpair isBST(Node *root){
    ...
    //We only need LeftMax and RightMin, but we donno if we are in right subrtree or in the left subtree
    //If current is a child, then its parent needs Max & if current is a left child and Min if current is a right child
    //Also to calculate curr min -> We need left min and right min (similarly for max)
    //Thiat is the reason we calculate both min and max for each node
